...
import { expectedGoals, overProb, bttsYesProb } from "../lib/model";
...
for (const m of matches) {
  ...
  const lambda = expectedGoals(favProb);
  const over = r2(overProb(lambda));
  ...
  if (m.marquee) {
    const aggLegs: BetSlipLeg[] = [
      ...
      },
      {
        ...
        probability: r2(bttsYesProb(favProb, lambda)),
        odds: io(r2(bttsYesProb(favProb, lambda))),
      },
    ];
    betSlips.push({
      id: `${m.id}-agg`,
